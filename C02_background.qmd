...
template_raster <- rast(obs, res = 0.5)

# 2. Thin the points
# This keeps exactly one random point per cell
thin_points <- spatSample(vect(obs), 
                           method = "random", 
...
ggplot() +
  geom_sf(data = obsbkg, 
          ...
          alpha =  0.08, shape = "circle small", size = 1) +
  geom_sf(data = coast, col = "orange")  + 
  labs(x = "Longitude", y = "Latitude", title = "All") +   
  theme_bw() +  # <- make a simple white background
  scale_color_okabe_ito() +  # <-- colorblind friendly for N Record
  facet_wrap(~month)
...
```{r save_model_input}
write_model_input(obsbkg, scientificname = "Squalus acanthias")
```

Note that there is a function for reading model input files, too.

```{r read_model_input}
x = read_model_input(scientificname = "Squalus acanthias")
```
